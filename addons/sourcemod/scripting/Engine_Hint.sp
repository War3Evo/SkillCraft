...
{
	name="SkillCraft Engine Hint Display",
	author="SkillCraft Team",
	description="SkillCraft Core Plugins",
	version="1.0",
};

new Handle:objarray[MAXPLAYERSCUSTOM][SC_HintPriority];
new bool:updatenextframe[MAXPLAYERSCUSTOM];
new String:lastoutput[MAXPLAYERSCUSTOM][129];

public bool:Init_SC_NativesForwards()
{
	CreateNative("SC_Hint",Native_SC_Hint);
	//SC_Hint(client,SC_HintPriority:type=HINT_LOWEST,Float:duration=5.0,String:format[],any:...);
	return true;
}
...
public On_SC_EventSpawn(client)
{
	...
	SC_Hint(client,_,0.1,"SkillCraft BETA"); //this is a hackish way of doing it, since it will not refresh if hint string is equal
...
public Native_SC_Hint(Handle:plugin,numParams)
{
	if(enabled)
	{
		...
		new SC_HintPriority:priority=SC_HintPriority:GetNativeCell(2);
		...
		if(SC_GetHintPriorityType(priority)==HINT_TYPE_SINGLE)
		{
			ClearArray(arr);
		}
		...
		for (new client = 1; client <= MaxClients; client++)
		{
			if (ValidPlayer(client,true)&&!IsFakeClient(client))
			{
				...
				if (lastshow[client] < time-0.3 || updatenextframe[client])
				{
					...
					output[0]=0;
					for (new SC_HintPriority:priority=HINT_NORMAL; priority < HINT_SIZE; priority++)
					{
						new Handle:arr=objarray[client][priority];
						if (arr != INVALID_HANDLE)
						{
							...
							if (size)
							{
								for (new arrindex=0;arrindex<size;arrindex+=2)
								{
									...
									else
									{
										...
										//DP("cat %s",str);
										if(SC_GetHintPriorityType(SC_HintPriority:priority)!=HINT_TYPE_ALL) //PRINT ONLY 1
										{
											break;
										}
									}
								}
								if (size&&SC_HintPriority:priority==HINT_NORMAL) //size may have changed when somethign expired
								{
									StrCat(output,sizeof(output)," \n");
								}
							}
					...
					if(strlen(output)>1 /*&& strcmp(output," ")==-1 && strcmp(output,"  ")==-1*/)
					{
						...
						{
						//DP("1");
							//PrintToServer("[SC_Hint] printing \"%s\"",output);
...
DeleteObject(client)
{
	...
	for(new SC_HintPriority:i=HINT_NORMAL; i < HINT_SIZE; i++)
	{
...
//do NOT print to chat here
	...
	if(enabled)
	{
		...
		//PrintToServer("[SC_Hint] recieved \"%s\"",str);
		...
		for (new i = 0; i < playersNum; i++)
		{
			if (players[i] != 0 && IsClientInGame(players[i]) && !IsFakeClient(players[i]))
			{
				...
				if (intercept)
				{
					SC_Hint(players[i],HINT_NORMAL,4.0,str); //causes update
					//urgent update
