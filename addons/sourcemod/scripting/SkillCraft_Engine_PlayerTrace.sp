...
{
	name="SkillCraft Engine Player Trace",
...
public bool:Init_SC_NativesForwards()
{
	...
	CreateNative("SC_GetAimEndPoint",Native_SC_GetAimEndPoint);
	CreateNative("SC_GetAimTraceMaxLen",Native_SC_GetAimTraceMaxLen);
	
	CreateNative("SC_GetTargetInViewCone",Native_SC_GetTargetInViewCone);
	
	CreateNative("SC_LOS",Native_SC_LOS);
	return true;
}

new ignoreClient;
public Native_SC_GetAimEndPoint(Handle:plugin,numParams)
{
...
public Native_SC_GetAimTraceMaxLen(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	new Float:angle[3];
	GetClientEyeAngles(client,angle);
...
public Native_SC_GetTargetInViewCone(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
			...
			if(ValidPlayer(i,true)&& client!=i)
			{
				if(FilterFunction!=INVALID_FUNCTION)
				{
					...
					if(Call_Finish(result)>SP_ERROR_NONE)
					{
						...
						PrintToServer("[War3Evo] ERROR in plugin \"%s\" traced to SC_GetTargetInViewCone(), bad filter function provided.",plugin_name);
					}
					if(result==0)
					{
						continue;
...
new los_target;
public Native_SC_LOS(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
