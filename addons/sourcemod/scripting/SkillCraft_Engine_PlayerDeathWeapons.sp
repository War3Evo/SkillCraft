...
{
	name="SkillCraft Engine Death Weapons",
...
public OnPluginStart()
{
	...
	{
		PrintToServer("[SkillCraft] Error finding weapon list offset.");
	}
	Clip1Offset=FindSendPropOffs("CBaseCombatWeapon","m_iClip1");
	if(Clip1Offset==-1)
	{
		PrintToServer("[SkillCraft] Error finding clip1 offset.");
	}
	AmmoOffset=FindSendPropOffs("CBasePlayer","m_iAmmo");
	if(AmmoOffset==-1)
	{
		PrintToServer("[SkillCraft] Error finding ammo offset.");
	}
}

public bool:Init_SC_NativesForwards()
{
	CreateNative("SC_CachedAngle",Native_SC_CachedAngle);
	CreateNative("SC_CachedPosition",Native_SC_CachedPosition);
	CreateNative("SC_CachedDucking",Native_SC_CachedDucking);
	CreateNative("SC_CachedWeapon",Native_SC_CachedWeapon);
	CreateNative("SC_CachedClip1",Native_SC_CachedClip1);
	CreateNative("SC_CachedAmmo",Native_SC_CachedAmmo);
	CreateNative("SC_CachedDeadClip1",Native_SC_CachedDeadClip1);
	CreateNative("SC_CachedDeadAmmo",Native_SC_CachedDeadAmmo);
	CreateNative("SC_CachedDeadWeaponName",Native_SC_CDWN);
	return true;
}


public Native_SC_CachedAngle(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);	
	SetNativeArray(2,pfAngle[client],3);
}

public Native_SC_CachedPosition(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	SetNativeArray(2,pfPos[client],3);
}

public Native_SC_CachedDucking(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	return (pbDucking[client])?1:0;
}

public Native_SC_CachedWeapon(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
...
public Native_SC_CachedClip1(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	new iter=GetNativeCell(2);
	if( iter>=0 && iter<10)
...
public Native_SC_CachedAmmo(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
...
public Native_SC_CachedDeadClip1(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	new iter=GetNativeCell(2);
	if( iter>=0 && iter<10)
...
public Native_SC_CachedDeadAmmo(Handle:plugin,numParams)
{
	new client=GetNativeCell(1);
	new id=GetNativeCell(2);
	if( id>=0 && id<32)
...
public Native_SC_CDWN(Handle:plugin,numParams) //cached weapon name?
{
	new client=GetNativeCell(1);
	new iter=GetNativeCell(2);
	if( iter>=0 && iter<10)
...
public On_SC_EventDeath(victim){
